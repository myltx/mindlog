...
- 🔧 修复博客详情页返回列表页时滚动位置异常的问题
- 🔧 修复从无 footer 的详情页返回有 footer 的列表页时页面停留在底部的问题

### 改进 (Improved)

- ✨ 添加自定义路由滚动行为，优化页面导航体验
- 🎯 从博客详情页返回列表页时自动滚动到顶部
- 🎯 保留浏览器前进/后退时的滚动位置记忆功能
- 🎨 返回按钮使用 navigateTo 替代 NuxtLink，更好的控制导航行为

### 技术细节 (Technical)

- 创建 `app/router.options.ts` 自定义路由滚动行为
- 实现智能滚动判断：
  - 浏览器前进/后退：恢复保存的滚动位置
  - 锚点导航：平滑滚动到锚点
  - 博客详情返回列表：滚动到顶部
  - 页面切换：默认滚动到顶部
- 所有滚动行为使用 `behavior: 'smooth'` 平滑过渡

### 文件变更

- `app/router.options.ts` - 新建自定义路由滚动配置
- `pages/blog/[...slug].vue` - 修改返回逻辑
- `nuxt.config.ts` - 清理冗余配置
...
[Unreleased]: https://github.com/myltx/nuxt-website/compare/v1.0.4...HEAD
[1.0.4]: https://github.com/myltx/nuxt-website/compare/v1.0.3...v1.0.4
